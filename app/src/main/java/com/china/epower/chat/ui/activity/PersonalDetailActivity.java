...
public class PersonalDetailActivity extends BaseActivity
  ...
  private void getLocalVCardRealm() {
    ...
    try {
      //测试1@dc-a4b8eb92-xmpp.jiangtao.tech./jiangtao
      Log.d(TAG, "getLocalVCardRealm: "+appPreferences.getString("userJid"));
      RealmResults<VCardRealm> realmResult =
          realmQuery.equalTo("jid", appPreferences.getString("userJid")).findAll();
      if (realmResult.size() != 0) {
        mVCardRealm = realmResult.first();
      }else {
        mVCardRealm = new VCardRealm(appPreferences.getString("userJid"));
  ...
  @Override protected void onActivityResult(int requestCode, int resultCode, Intent data) {
    ...
    if (resultCode == RESULT_OK) {
      ...
      if (list.size() != 0) {
        Log.d(TAG, "onActivityResult: 判断mVCardRealm是否为null"+(mRealm==null));
        if (mVCardRealm != null) {
