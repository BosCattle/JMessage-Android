  ...
  public class SupportService extends Service
          ...
          .subscribe(affiliates -> {
            LogUtils.d(TAG,"获得信息");
              for (Affiliate affiliate:affiliates){
                ...
                LogUtils.d(TAG,affiliate.getRole().name());
              }
          });
    }
    ...
    public void getRoomsInformation() {
      ...
      Observable.create((Observable.OnSubscribe<List<String>>) subscriber -> {
        try {
          subscriber.onNext(mMultiUserChatManager.getJoinedRooms(mXMPPConnection.getUser()));
        } catch (SmackException.NoResponseException | SmackException.NotConnectedException | XMPPException.XMPPErrorException e) {
          e.printStackTrace();
          subscriber.onError(e);
        }
      }).subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(rooms -> {
        LogUtils.d(TAG, "所有群组" + rooms.toString());
        for (String s : rooms) {
          ...
          Observable.create((Observable.OnSubscribe<RoomInfo>) subscriber -> {
            try {
              ...
              .subscribe(roomInfo -> {
                ...
                LogUtils.d(TAG, roomInfo.getSubject() + "");
                // 保存所有群组
                IMRoomStoreModel model = new IMRoomStoreModel(roomInfo.getRoom(), roomInfo.getName(),
                    roomInfo.getDescription(),
                    roomInfo.getForm().getField("avatar").getValues().get(0),
                    roomInfo.getContactJids());
                LogUtils.i(TAG, Arrays.toString(roomInfo.getContactJids().toArray()));
                HermesEventBus.getDefault().postSticky(model);
              }, new ErrorAction() {
                @Override public void call(Throwable throwable) {
                  super.call(throwable);
